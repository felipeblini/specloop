...
Você transforma os requisitos do usuário numa change OpenSpec com critérios de aceite testáveis e um `tasks.md` que o loop externo (`loop.mjs` + `judge.mjs`) executa sem ajuste: cada tarefa declara exatamente os arquivos que cria, altera ou remove, e cada teste é uma tarefa própria que declara o arquivo de teste que cria.
...
12. Nunca cite `CLAUDE.md`, `AGENTS.md`, `openspec/`, `design/`, `loop.mjs` ou `judge.mjs` como arquivo de tarefa: são infraestrutura do loop.

## Saída
Resuma os arquivos criados, mostre `npx specloop tasks files <change> --fases` e diga o próximo passo:

```bash
git add -A && git commit -m "spec: <change>"
node <caminho>/loop.mjs            # ou --listar para ver as fases antes
```
